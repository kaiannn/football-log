...
_MODELS = ["yolov8n.pt", "yolov8s.pt", "yolov8m.pt", "yolov8l.pt", "yolov8x.pt"]
...
def _run_video_streaming(
    ...
    output_files = _collect_output_files(pipeline.data_writer.output_dir)
    ...
    yield rgb, output_files, summary
    ...
    with gr.Blocks(title="Football Log") as app:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                with gr.Tab("视频文件"):
                    video_input = gr.Video(label="比赛视频")
                ...
                gr.Markdown("### 检测参数")
                model = gr.Dropdown(_MODELS, value="yolov8n.pt", label="YOLO 模型")
            ...
            with gr.Column(scale=2):
                gr.Markdown("### 结果")
                preview = gr.Image(label="预览", height=480)
                status = gr.Textbox(label="状态", interactive=False)
                output_files = gr.File(label="导出文件", file_count="multiple", interactive=False)
        ...
        outputs = [preview, output_files, status]
