...
│   Keypoint + LAB     → 姿态关键点取躯干，LAB 颜色投票            │
...
| 维度 | 最佳 | 可用 | 不推荐 |
|------|------|------|--------|
| 格式 | MP4 (H.264) | AVI, MOV | MKV, WebM |
| 分辨率 | 1920×1080 | 1280×720 | <720p, 4K |
| 帧率 | 25/30 fps | 50/60 fps | <20 fps |
| 镜头 | 转播半场高角度机位 | 全场战术机位 | 特写/底线/航拍 |
| 内容 | 正常比赛画面 | 夜间比赛 | 回放/慢动作/字幕叠加 |
| 标定 | 关键点模型（自动） | 手标关键帧 | 无标定 |

**最不适配**：低分辨率（<720p）+ 特写镜头 + 无标定 + 回放画面混剪 — 检测率低、跟踪碎片化、世界坐标不可用。
